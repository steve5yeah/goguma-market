...
export default function SignupForm() {
  ...
  if (state.notice) {
    return (
      <div className="space-y-3 text-center">
        ...
        <p className="font-semibold text-soil-900">메일을 확인해 주세요</p>
        <p className="text-sm leading-relaxed text-soil-600">{state.notice}</p>
      </div>
    );
  }
  ...
  return (
    <form action={formAction} className="space-y-4">
      <div>
        <label htmlFor="email" className="mb-1.5 block text-sm font-medium text-soil-800">
          이메일
        </label>
        <input
          id="email"
      ...
      <div>
        <label htmlFor="password" className="mb-1.5 block text-sm font-medium text-soil-800">
          비밀번호
        </label>
        <input
          id="password"
